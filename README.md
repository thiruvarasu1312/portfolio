# Thiruvarasu – AI & Data Science Portfolio

Welcome to my portfolio repository!
I am a **third-year B.Tech Artificial Intelligence & Data Science student** passionate about building AI-powered solutions and solving real-world problems using data, machine learning, and intelligent systems.

---

## 👨‍💻 About Me

* 🎓 B.Tech – Artificial Intelligence & Data Science
* 🏫 Kumaraguru College of Technology (2023 – 2027)
* 📊 CGPA: 7.2
* 💡 Interested in Machine Learning, Neural Networks, and AI Applications
* 🚀 Passionate about building real-world AI solutions

---

## 🧠 Technical Skills

![Skills] 

* Python
* Machine Learning
* Artificial Intelligence
* Neural Networks
* C / C++ (Basics)
* SQL (Basics)
* Power BI
* Data Structures
* DBMS Fundamentals
* Operating System Concepts

---

## 📂 Projects

### ⚡ Power Monitoring Web Application

Developed a web application to monitor **voltage, current, and power consumption of a conveyor motor**.

**Technologies Used**

* PHP
* MySQL
* HTML
* CSS
* JavaScript

---

### 🧠 Brain Tumor Classification

Developed an **AI-powered MRI brain tumor classification system** with explainable AI techniques.

**Technologies Used**

* PyTorch
* Flask
* Grad-CAM
* LIME

---

### 📰 Fake News Detection System

Built a **machine learning model to classify news as real or fake**.

**Technologies Used**

* Python
* Scikit-learn
* NLP techniques

---

## 🏢 Startup Experience

Worked with **Autosegra Solution** on a project focused on **medical waste segregation**, contributing to safer and more efficient waste management systems.

---

## 📜 Certifications

* Data Mining – NPTEL
* Introduction to Cloud Computing – IBM
* Introduction to Artificial Intelligence – Infosys Springboard
* Introduction to NLP – Infosys Springboard

---

## 📫 Contact

📧 Email: [thiruvarasu...@gmail.com](mailto:thiruvarasu...@gmail.com)
💼 LinkedIn: https://linkedin.com/in/thiruvarasu
📱 Phone: 6374244187

---

⭐ If you like my work, feel free to explore my projects and connect with me!
